...
async def test_malformed_frames_get_clean_errors(tmp_path):
    ...
    async with serve(server.handle, "localhost", 8791):
        ...
        for i, (label, frame, expected) in enumerate(MALFORMED_FRAMES):
            ...
            # Use a UNIQUE nickname per case. The server binds a nickname to the
            # first identity key that claims it (anti-squatting), so reusing a
            # name across cases -- several labels share the same first characters --
            # with a fresh identity would be correctly rejected, which is not what
            # this test is checking. The index keeps recovery names distinct.
            await ws.send(json.dumps({
                "type": "hello", "nick": f"recover{i}",
